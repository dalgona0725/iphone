//
//  AutoSlidingLabel.swift
//  PopkonAir
//
//  Created by Eugene Jeong on 2018. 2. 1..
...
open class AutoSlidingLabel: UILabel, CAAnimationDelegate {

    /**
     An enum that defines the types of `AutoSlidingLabel` scrolling
    ...
    /**
     Defines the direction and method in which the `AutoSlidingLabel` instance scrolls.
     `AutoSlidingLabel` supports six default types of scrolling: `Left`, `LeftRight`, `Right`, `RightLeft`, `Continuous`, and `ContinuousReverse`.
     ...
     */
    open var type: SlidingType = .continuous {
        didSet {
            if type == oldValue {
                return
            }
    ...
    /**
     ...
     - Attention: Use of the `scrollSequence` property requires understanding of how AutoSlidingLabel works for effective
    ...
    /**
     A boolean property that sets whether the `AutoSlidingLabel` should behave like a normal `UILabel`.
     
     When set to `true` the `AutoSlidingLabel` will behave and look like a normal `UILabel`, and  will not begin any scrolling animations.
     Changes to this property take effect immediately, removing any in-flight animation as well as any edge fade. Note that `AutoSlidingLabel`
    ...
    /**
     A boolean property that sets whether the `AutoSlidingLabel` should hold (prevent) automatic label scrolling.
     
     When set to `true`, `AutoSlidingLabel` will not automatically scroll even its text is larger than the specified frame,
     although the specified edge fades will remain.
     
     To set `AutoSlidingLabel` to act like a normal UILabel, use the `labelize` property.
    ...
    /**
     A boolean property that sets whether the `AutoSlidingLabel` should only begin a scroll when tapped.
     
     If this property is set to `true`, the `AutoSlidingLabel` will only begin a scroll animation cycle when tapped. The label will
    ...
    @IBInspectable open var tapToScroll: Bool = false {
        didSet {
            if tapToScroll != oldValue {
                if tapToScroll {
                    let tapRecognizer = UITapGestureRecognizer(target: self, action: #selector(AutoSlidingLabel.labelWasTapped(_:)))
    ...
    /**
     The `AutoSlidingLabel` scrolling speed may be defined by one of two ways:
    ...
    /**
     Defines the speed of the `AutoSlidingLabel` scrolling animation.
    ...
    /**
     ...
     This propery sets the size (in points) of the view edge transparency fades on the left and right edges of a `AutoSlidingLabel`. The
    ...
    /**
     Convenience method to restart all `AutoSlidingLabel` instances that have the specified view controller in their next responder chain.
     
     - Parameter controller: The view controller for which to restart all `AutoSlidingLabel` instances.
     
     - Warning: View controllers that appear with animation (such as from underneath a modal-style controller) can cause some `AutoSlidingLabel` text
    ...
    open class func restartLabelsOfController(_ controller: UIViewController) {
        AutoSlidingLabel.notifyController(controller, message: .Restart)
    }
    ...
    open class func controllerViewWillAppear(_ controller: UIViewController) {
        AutoSlidingLabel.restartLabelsOfController(controller)
    }
    ...
    /**
     Labelizes all `AutoSlidingLabel` instances that have the specified view controller in their next responder chain.
     
     The `labelize` property of all recognized `AutoSlidingLabel` instances will be set to `true`.
     
     - Parameter controller: The view controller for which all `AutoSlidingLabel` instances should be labelized.
     - SeeAlso: labelize
     */
    open class func controllerLabelsLabelize(_ controller: UIViewController) {
        AutoSlidingLabel.notifyController(controller, message: .Labelize)
    }
    
    /**
     De-labelizes all `AutoSlidingLabel` instances that have the specified view controller in their next responder chain.
     
     The `labelize` property of all recognized `AutoSlidingLabel` instances will be set to `false`.
     
     - Parameter controller: The view controller for which all `AutoSlidingLabel` instances should be de-labelized.
     - SeeAlso: labelize
     */
    open class func controllerLabelsAnimate(_ controller: UIViewController) {
        AutoSlidingLabel.notifyController(controller, message: .Animate)
    }
    ...
    /**
     Returns a newly initialized `AutoSlidingLabel` instance with the specified scroll rate and edge transparency fade length.
     ...
     - Parameter fadeLength: A length of transparency fade at the left and right edges of the `AutoSlidingLabel` instance's frame.
     - Returns: An initialized `AutoSlidingLabel` object or nil if the object couldn't be created.
     - SeeAlso: fadeLength
     */
    ...
    /**
     Returns a newly initialized `AutoSlidingLabel` instance.
    ...
    private func setup() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(self.restartForViewController(_:)), name: NSNotification.Name(rawValue: MarqueeKeys.Restart.rawValue), object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(self.labelizeForController(_:)), name: NSNotification.Name(rawValue: MarqueeKeys.Labelize.rawValue), object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(self.animateForController(_:)), name: NSNotification.Name(rawValue: MarqueeKeys.Animate.rawValue), object: nil)
        // UIApplication state notifications
//        NotificationCenter.default.addObserver(self, selector: #selector(self.restartLabel), name: UIApplication.didBecomeActiveNotification, object: nil)
//        NotificationCenter.default.addObserver(self, selector: #selector(self.shutdownLabel), name: UIApplication.didEnterBackgroundNotification, object: nil)
    }
    ...
    //
    // MARK: - AutoSlidingLabel Heavy Lifting
    //
    ...
    private func updateAndScroll(_ shouldBeginScroll: Bool) {
        ...
        // Invalidate intrinsic size. 고유한 사이즈이 아님을 알리다.
        invalidateIntrinsicContentSize()
        
        // Move label to home. 라벨을 원래의 자리로 보낸다.
//        returnLabelToHome()
        
        // Check if label should scroll
        // Note that the holdScrolling propery does not affect this
        // 스크롤 가능하지 않은 라벨이라면 일반 라벨처럼 처리한다.
    ...
    private func transactionDurationType(_ labelType: SlidingType, interval: CGFloat, delay: CGFloat) -> TimeInterval {
        switch (labelType) {
    ...
    override open var numberOfLines: Int {
        ...
        set {
            // By the nature of AutoSlidingLabel, this is 1
            super.numberOfLines = 1
        }
    }
    ...
    override open var adjustsFontSizeToFitWidth: Bool {
        ...
        set {
            // By the nature of AutoSlidingLabel, this is false
...
public struct ScrollStep: MarqueeStep {
    /**
     ...
     The `away` position depends on the AutoSlidingLabel `type` value.
...
fileprivate extension CAMediaTimingFunction {
    ...
    func solveTforY(_ y_0: CGFloat, epsilon: CGFloat, controlPoints: [CGPoint]) -> CGFloat {
        ...
        print("AutoSlidingLabel: Failed to find t for Y input!")
        return t0
    }
...
open class SingleShotOffscreenLabel: AutoSlidingLabel {
    
    var scrollDoneBlock : () -> Void = { }
...
//        super.layoutSubviews()
    ...
    open override func labelWillBeginScroll() {
        ...
        if type == .leftRight || type == .rightLeft {
            ...
            reservedTimer = Timer.scheduledTimer(timeInterval: TimeInterval(awayTime), target: self, selector: #selector(pauseAndClear), userInfo: nil, repeats: false)
            }
        }
    }
...
open class NoAnimationReturn: AutoSlidingLabel {
    // Override labelWillBeginScroll to catch when a scroll animation starts
...
open class SingleShotResetLabel: AutoSlidingLabel {
    // Override labelWillBeginScroll to catch when a scroll animation starts
    override open func labelWillBeginScroll() {
        // This only makes sense for leftRight and rightLeft types
        if type == .leftRight || type == .rightLeft {
            ...
            let awayTime = animationDelay + animationDuration
            // Schedule a timer to restart the label when it hits the "away" position
            Timer.scheduledTimer(timeInterval: TimeInterval(awayTime), target: self, selector: #selector(shutdownLabel), userInfo: nil, repeats: false)
        }
    }
}
